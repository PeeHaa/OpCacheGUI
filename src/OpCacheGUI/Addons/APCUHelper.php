...
class APCUHelper
{
    ...
    public static function textArc($im, $centerX, $centerY, $diameter, $start, $end, $color1, $text, $placeindex = 0)
    {
        $r = $diameter / 2;
        $w = deg2rad((360 + $start + ($end - $start) / 2) % 360);
    ...
    public static function fillBox($im, $x, $y, $w, $h, $color1, $color2, $text = '', $placeindex = '')
    {
        global $col_black;
        $x1 = $x + $w - 1;
        $y1 = $y + $h - 1;
    ...
    public static function putloginLink($s = "Login")
    {
        ...
        switch ($imgid) {
            case 1:
                    ...
                    foreach ($free as $block) {
                        if ($block['offset'] != $ptr) {
                            ...
                            if (($angle_to * 360) - ($angle_from * 360) >= 1) {
                                self::fillArc($image, $x, $y, $size, $angle_from * 360, $angle_to * 360, $col_black, $col_red);
                        ...
                        if (($angle_to * 360) - ($angle_from * 360) >= 1) {
                            self::fillArc($image, $x, $y, $size, $angle_from * 360, $angle_to * 360, $col_black, $col_green);
                            if (($angle_to - $angle_from) > 0.05) {
                                array_push($string_placement, array($angle_from, $angle_to));
                            }
                ...
                foreach ($string_placement as $angle) {
                    self::textArc($image, $x, $y, $size, $angle[0] * 360, $angle[1] * 360, $col_black, self::bsize($s * ($angle[1] - $angle[0])));
                }
                break;
            case 2:
                $cache['num_hits'] = isset($cache['num_hits']) ? $cache['num_hits'] : $cache['nhits'];
                ...
                self::fillBox($image, 30, $size, 50, $s ? (-$a * ($size - 21) / $s) : 0, $col_black, $col_green, sprintf("%.1f%%", $s ? $cache['num_hits'] * 100 / $s : 0));
                self::fillBox($image, 130, $size, 50, $s ? -max(4, ($s - $a) * ($size - 21) / $s) : 0, $col_black, $col_red, sprintf("%.1f%%", $s ? $cache['num_misses'] * 100 / $s : 0));
                break;
            case 3:
                $s = $mem['num_seg'] * $mem['seg_size'];
                $a = $mem['avail_mem'];
                ...
                for ($i = 0; $i < $mem['num_seg']; $i++) {
                    ...
                    foreach ($free as $block) {
                        if ($block['offset'] != $ptr) {
                            ...
                            if ($h > 0) {
                                $j++;
                                if ($j < 75) {
                                    self::fillBox($image, $x, $y, 50, $h, $col_black, $col_red, self::bsize($block['offset'] - $ptr), $j);
                                } else {
                        ...
                        if ($h > 0) {
                            $j++;
                            if ($j < 75) {
                                self::fillBox($image, $x, $y, 50, $h, $col_black, $col_green, self::bsize($block['size']), $j);
                            } else {
                    ...
                    if ($ptr < $mem['seg_size']) {
                        ...
                        if ($h > 0) {
                            self::fillBox($image, $x, $y, 50, $h, $col_black, $col_red, self::bsize($mem['seg_size'] - $ptr), $j++);
                        }
                    }
                }
                break;
