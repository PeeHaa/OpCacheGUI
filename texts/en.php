...
$texts = [
    ...
    'apc.enabled' => 'apc.enabled can be set to 0 to disable APC. This is primarily useful when APC is statically compiled into PHP, since there is no other way to disable it (when compiled as a DSO, the extension line in php.ini can just be commented-out).',
    'apc.enable_cli' => 'Mostly for testing and debugging. Setting this enables APC for the CLI version of PHP. Under normal circumstances, it is not ideal to create, populate and destroy the APC cache on every CLI request, but for various test scenarios it is useful to be able to enable APC for the CLI version of PHP easily.',
    'apc.coredump_unmap' => 'Enables APC handling of signals, such as SIGSEGV, that write core files when signaled. When these signals are received, APC will attempt to unmap the shared memory segment in order to exclude it from the core file. This setting may improve system stability when fatal signals are received and a large APC shared memory segment is configured.',
    'apc.entries_hint' => 'a "hint" about the number of distinct user cache variables to store. Set to zero or omit if not sure.',
    ...
    'apc.mmap_file_mask' => 'If compiled with MMAP support by using --enable-mmap this is the mktemp-style file_mask to pass to the mmap module for determining whether your mmap ed memory region is going to be file-backed or shared memory backed. For straight file-backed mmap, set it to something like /tmp/apc.XXXXXX (exactly 6 Xs). To use POSIX-style shm_open/mmap put a .shm somewhere in your mask. e.g. /apc.shm.XXXXXX You can also set it to /dev/zero to use your kernel\'s /dev/zero interface to anonymous mmap\'ed memory. Leaving it undefined will force an anonymous mmap.',
    'apc.preload_path' => 'Optionally, set a path to the directory that APC will load cache data at startup.',
    'apc.rfc1867' => 'RFC1867 File Upload Progress hook handler is only available if APC was compiled against PHP 5.2.0 or later. When enabled, any file uploads which includes a field called APC_UPLOAD_PROGRESS before the file field in an upload form will cause APC to automatically create an upload_key user cache entry where key is the value of the APC_UPLOAD_PROGRESS form entry.',
    'apc.use_request_time'=> 'Use the SAPI request start time for TTL.',
    'apc.file_md5'=> 'Records a md5 hash of files.',
    'apc.lazy_functions'=> 'Enables lazy loading for functions.',
    'apc.lazy_classes'=> 'Enables lazy loading for classes.',
    'apc.rfc1867_prefix'=> 'Key prefix to use for the user cache entry generated by rfc1867 upload progress functionality.',
    'apc.rfc1867_name'=> 'Specify the hidden form entry name that activates APC upload progress and specifies the user cache key suffix.',
    'apc.rfc1867_freq' => 'The frequency that updates should be made to the user cache entry for upload progress. This can take the form of a percentage of the total file size or a size in bytes optionally suffixed with "k", "m", or "g" for kilobytes, megabytes, or gigabytes respectively (case insensitive). A setting of 0 updates as often as possible, which may cause slower uploads.',
    'apc.rfc1867_ttl'=> 'TTL for rfc1867 entries.',
    'apc.localcache'=> 'This enables a lock-free local process shadow-cache which reduces lock contention when the cache is being written to.',
    ...
    'apc.serializer' => 'Used to configure APC to use a third party serializer.',
    'apc.use_request_time' => 'Use the SAPI request start time for TTL.',
    'apc.writable' => '(undocumented) I guess the name says it all',
    'apc.smart' => '(undocumented)',
    'apc.slam_defense' => 'On very busy servers whenever you start the server or modify files you can create a race of many processes all trying to cache the same file at the same time. This option sets the percentage of processes that will skip trying to cache an uncached file. Setting this to 0 disables this feature.',
    'apc.shm_segments' => 'The number of shared memory segments to allocate for the compiler cache. If APC is running out of shared memory but apc.shm_size is set as high as the system allows, raising this value might prevent APC from exhausting its memory.',
